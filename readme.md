...
4. 第二个宏为四个基本类型重载了一些算术运算的操作符
   ...
   由于没有考试之后多了一堆的作业, 并且os实验这边还没有给出题目. 剩下的3个lab应该能在一个星期内完成, 当然还要写总结报告, 还得改进一点什么东西. 所以预留10天的时间, 这几天完成一下学校的作业和实验.
   
   今天上午迅速过一遍第456个实验做啥。

下图是刚刚看memoryset.rs的时候做的图, 能够理清思路
...
学校这边的作业暂时还有一个大作业(做一个搜索引擎),以及信息检索的作业7,8

今天不打算开始新的实验了, 今天下午和晚上写一下前三个实验的学习报告, 这个月的后十天就做后面的三个实验, 以及写一下置换算法或者调度算法.
...
从明天开始做第三个实验开始的实验题，第一第二个已经在之前做好了。

并且增加一些自己写的调度算法。

本来的文件里有fifo和最高响应比优先算法

打算完成： 短进程优先（预估执行时间、时间片轮转算法(Round Robin， 直接变成BFS算法)、多级反馈队列、公平共享调度算法、
...
今天实现时间片轮转算法的时候, 发现原来的代码就相当于已经实现了。一个fifo加上时钟中断，相当于一个时间片轮转了，还误导了我好久。

然后又开始写短进程优先算法，发现在目前的框架下如果要实现短进程优先算法比较麻烦， 因为要记录之前的进程执行的时间，这样就意味着时间片的大小不应该是相同的， 但是由于目前框架的时间片大小是相同的，导致无法实现。

也写了多级反馈优先算法的简化版本，同样因为时间片的大小是相同的，导致无法区分出优先级，所以只能设置两个队列，前台是时间片轮转算法，后台是hrrn算法。如果某一个线程执行一个时间片后还没有结束， 就将他移到后台。这只能说有多级反馈队列的雏形了。

明天想办法设置一下可变时间片，然后就可以优化一下今天写的算法了。



2020年7月22日20:40:17

和同学讨论了一下, 决定在调度算法里修改时钟中断的间隔,这样就可以获得不同的时间片。然后通过设置不同的优先级，可以对线程进行调度。 然后用if let语句进程判断不同的优先级队列中是否有线程。
